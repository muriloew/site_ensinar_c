...
from backend.banco.conexao import transacao, usando_postgres

CHAVE_PRIMARIA_SQLITE = "INTEGER PRIMARY KEY AUTOINCREMENT"
CHAVE_PRIMARIA_POSTGRES = "INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY"
...
def criar_tabelas():
    ...
    chave = CHAVE_PRIMARIA_POSTGRES if postgres else CHAVE_PRIMARIA_SQLITE
    with transacao() as conn:
        for sql in TABELAS:
            conn.execute(sql.format(chave=chave))
        if postgres:
            for tabela, coluna, definicao in COLUNAS_ADICIONADAS:
                conn.execute(f"ALTER TABLE {tabela} ADD COLUMN IF NOT EXISTS {coluna} {definicao}")
        else:
