"""Compila e executa programas C dos alunos com limites de tempo, memória e processos."""
...
def _sinais_conhecidos():
    descricoes = {
        "SIGSEGV": "falha de segmentação: acesso inválido à memória",
        "SIGFPE": "erro aritmético, como divisão inteira por zero",
        "SIGABRT": "programa abortado",
        "SIGXCPU": "limite de tempo de CPU excedido",
        "SIGKILL": "encerrado pelo limite de tempo ou de memória",
        "SIGTERM": "encerrado pelo servidor",
    }
    return {
        int(getattr(signal, nome)): texto
        for nome, texto in descricoes.items()
        if hasattr(signal, nome)
    }


SINAIS_CONHECIDOS = _sinais_conhecidos()


def texto_retorno(codigo, motivo=""):
    """Linha final no estilo Code::Blocks, explicando quando o programa foi interrompido."""
    explicacao = motivo or (SINAIS_CONHECIDOS.get(-codigo, "") if codigo is not None and codigo < 0 else "")
    if explicacao:
        return f"Process returned {codigo} ({explicacao})"
    return f"Process returned {codigo}"
...
def _compilar_workspace(temp_dir, arquivo_c, arquivo_saida):
    # Nomes relativos ao diretório temporário deixam o build log com "programa.c:3:5".
    compiladores = _comandos_compiladores(
        os.path.basename(arquivo_c), os.path.basename(arquivo_saida)
    )
    if not compiladores:
        return {
            "ok": False,
